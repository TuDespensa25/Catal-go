...
// Componente de imagen optimizado con placeholder mejorado
const OptimizedImage = React.memo(({ src, alt, className, ...props }) => {
  ...
  const imageSrc = hasError 
    ? 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 100 100"><rect width="100" height="100" fill="%23f0f0f0"/><text x="50" y="50" font-size="40" text-anchor="middle" dy=".3em" fill="%23999">📦</text></svg>'
    : src;
...
// ==================== BOTÓN DE NOTIFICACIONES CORREGIDO (SIN process.env) ====================
const NotificationButton = React.memo(() => {
  ...
  const runDiagnostic = () => {
    ...
    console.log('- Token localStorage:', localStorage.getItem('fcm_token'));
    console.log('- Hora token:', localStorage.getItem('fcm_token_time') ? new Date(parseInt(localStorage.getItem('fcm_token_time'))).toLocaleString() : 'No guardado');
    console.log('- Firebase messaging:', !!firebase?.messaging);
    console.log('- requestNotificationPermission disponible:', !!window.requestNotificationPermission);
  ...
  return (
    <div className="fixed bottom-40 right-6 z-50">
      {/* Botón principal */}
      <button
        onClick={permission === 'granted' ? runDiagnostic : handleEnableNotifications}
        ...
        title={
          permission === 'granted' 
            ? 'Notificaciones activadas (clic para diagnosticar)' 
            : permission === 'denied'
