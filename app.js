// ==================== VERSIÓN CON CDN DE FIREBASE (SIN IMPORTACIONES) ====================
...
// Inicializar Firebase usando la versión global
firebase.initializeApp(firebaseConfig);
const messaging = firebase.messaging();

const VAPID_KEY = "BHtRjDmjawGcjLGnE37RLglg5wCriZmD7cJbbdkhXhDf6fMkKre01uUaF62csfozeidoANJ549AxVIt0Q38rW24";
  ...
  React.useEffect(() => {
    const checkSubscription = async () => {
      try {
        ...
        setIsSubscribed(!!(subscription && hasToken));
      } catch (error) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-sm w-full p-6">
            ...
            <p className="text-[#64748b] text-center mb-6">
              {isSubscribed 
                ? 'Recibirás ofertas y novedades'
                : 'Te avisaremos de ofertas especiales'}
            </p>
...
// TODO: Copia aquí TODO el código que tenías después del botón de notificaciones
// (municipalities, categories, productData, componentes, etc.)

// Por razones de espacio no lo repito, pero DEBES MANTENER todo el código
// que tenías desde la línea donde defines 'municipalities' hasta el final
// ==================== DATA ====================
...
const MUNICIPIOS_GRUPO_1 = [1, 2];
const MUNICIPIOS_GRUPO_2 = [4, 5, 6, 7, 8, 9, 10, 11];
...
function isProductAvailable(product, municipalityId) {
  if (Array.isArray(product.availableIn) && product.availableIn.length === 0) return false;
  if (product.availableIn === 1) return MUNICIPIOS_GRUPO_1.includes(municipalityId);
  if (product.availableIn === 2) return MUNICIPIOS_GRUPO_2.includes(municipalityId);
  if (Array.isArray(product.availableIn) && product.availableIn.includes(1) && product.availableIn.includes(2)) return true;
  return false;
}
...
function getProductFromURL() {
  ...
  if (productId) return productData.find(p => p.id === parseInt(productId));
  return null;
}
...
const OptimizedImage = React.memo(({ src, alt, className, ...props }) => {
  ...
  return (
    <div className={`relative ${className}`}>
      {isLoading && <div className="absolute inset-0 image-loading rounded" />}
      <img
        src={src}
        alt={alt}
        className={`${className} ${isLoading ? 'opacity-0' : 'opacity-100 fade-in'} transition-opacity duration-300`}
        onLoad={() => setIsLoading(false)}
        onError={(e) => { setIsLoading(false); setHasError(true); e.target.style.display = 'none'; }}
        loading="lazy"
        {...props}
      />
      {hasError && (
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl max-w-md w-full overflow-hidden animate-scale-in">
        <div className="bg-gradient-to-r from-[#10b981] to-[#059669] p-6 text-white text-center">
          ...
          <p className="text-white text-opacity-90">Elige tu ubicación para ver los productos disponibles en tu área</p>
        </div>
            ...
            {municipalities.map(municipality => (
              ...
              >
                ...
                <div className="flex-1">
                  <h3 className="font-semibold text-[#0f172a] text-lg">{municipality.name.split(', ')[1]}</h3>
                  <p className="text-sm text-[#64748b]">{municipality.name.split(', ')[0]}</p>
                </div>
                {selectedMunicipality === municipality.id && (
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-700 mt-1">Los productos disponibles y precios pueden variar según tu municipio seleccionado.</p>
              </div>
            </div>
          </div>
          <button onClick={onClose} className="w-full bg-gray-100 text-[#0f172a] py-3 rounded-full font-medium hover:bg-gray-200 transition-all">Cerrar</button>
        </div>
      </div>
    </div>
  );
...
const CartModal = React.memo(({ isOpen, onClose, cart, setCart, onProcessOrder }) => {
  const [customerData, setCustomerData] = React.useState({ name: '', phone: '', beneficiaryName: '', beneficiaryPhone: '', address: '', notes: '' });
  const [discountCode, setDiscountCode] = React.useState('');
  const [discountApplied, setDiscountApplied] = React.useState(false);
  const [discountError, setDiscountError] = React.useState('');
  const [discountPercentage, setDiscountPercentage] = React.useState(0);

  const validDiscountCodes = { 'CLIENTEF': 5, 'BIENVENIDOhoy': 10, 'PRIMERACOMPRAtd25.': 15 };

  const applyDiscount = () => {
    const code = discountCode.trim();
    if (!code) { setDiscountError('Por favor ingresa un código'); return; }
  ...
  const removeItem = (productId) => setCart(prevCart => prevCart.filter(item => item.id !== productId));

  const getTotalItems = () => cart.reduce((sum, item) => sum + item.quantity, 0);
  const getSubtotalPrice = () => cart.reduce((sum, item) => sum + (item.price * item.quantity), 0);
  const getDiscountAmount = () => (getSubtotalPrice() * discountPercentage) / 100;
  const getTotalPrice = () => getSubtotalPrice() - getDiscountAmount();
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-end sm:items-center sm:justify-center">
      <div className="bg-white w-full max-w-md max-h-[90vh] overflow-y-auto rounded-t-3xl sm:rounded-3xl">
        <div className="sticky top-0 bg-white border-b p-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-[#0f172a] flex items-center gap-2"><span>🛒</span> Mi Carrito ({getTotalItems()})</h2>
          <button onClick={onClose} className="p-1 hover:bg-gray-100 rounded-full transition"><span className="text-xl text-[#64748b]">✕</span></button>
        </div>

        {cart.length === 0 ? (
          <div className="p-8 text-center">
            <div className="w-16 h-16 mx-auto mb-4 bg-gray-100 rounded-full flex items-center justify-center"><span className="text-3xl text-gray-400">🛒</span></div>
          ...
          <>
            <div className="p-4 space-y-3">
              {cart.map(item => (
                <div key={item.id} className="flex items-center space-x-3 bg-gray-50 rounded-2xl p-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm font-semibold text-[#10b981]">${item.price.toFixed(2)} USD</p>
                  </div>
                  <div className="flex flex-col items-center space-y-1">
                    <div className="flex items-center space-x-2">
                      <button onClick={() => updateQuantity(item.id, -1)} className="w-8 h-8 rounded-full bg-white border flex items-center justify-center hover:bg-gray-50 transition"><span className="text-sm text-[#64748b]">−</span></button>
                      <span className="text-sm font-medium w-8 text-center text-[#0f172a]">{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.id, 1)} className="w-8 h-8 rounded-full bg-white border flex items-center justify-center hover:bg-gray-50 transition"><span className="text-sm text-[#64748b]">+</span></button>
                    </div>
                    <p className="text-xs font-medium text-[#0f172a]">${(item.price * item.quantity).toFixed(2)}</p>
                  </div>
                </div>
              ))}
              ...
              <div className="border-t pt-4 mt-4">
                <div className="mb-3">
                  <h3 className="text-sm font-medium text-[#0f172a] mb-2 flex items-center gap-1"><span>🏷️</span> Código de Descuento</h3>
                  <div className="flex space-x-2">
                    <input type="text" placeholder="Ingresa tu código" value={discountCode} onChange={(e) => setDiscountCode(e.target.value)} disabled={discountApplied} className="flex-1 p-2 border rounded-full focus:outline-none focus:ring-2 focus:ring-[#10b981] disabled:bg-gray-100" />
                    {!discountApplied ? (
                      <button onClick={applyDiscount} className="bg-[#10b981] text-white px-4 py-2 rounded-full font-medium hover:bg-opacity-90 transition-all">Aplicar</button>
                    ) : (
                      <button onClick={removeDiscount} className="bg-red-500 text-white px-4 py-2 rounded-full font-medium hover:bg-red-600 transition-all">Quitar</button>
                    )}
                  </div>
                  {discountError && <p className="text-red-500 text-xs mt-1">{discountError}</p>}
                  {discountApplied && <p className="text-green-600 text-xs mt-1">✅ Descuento del {discountPercentage}% aplicado correctamente</p>}
                </div>

                <div className="space-y-2 bg-gray-50 rounded-2xl p-3">
                  <div className="flex justify-between text-sm"><span className="text-[#64748b]">Subtotal:</span><span className="text-[#0f172a] font-medium">${getSubtotalPrice().toFixed(2)} USD</span></div>
                  {discountApplied && (
                    <>
                      <div className="flex justify-between text-sm text-green-600"><span>Descuento ({discountPercentage}%):</span><span>-${getDiscountAmount().toFixed(2)} USD</span></div>
                      <div className="border-t border-gray-200 pt-2 mt-2"><div className="flex justify-between items-center"><span className="text-lg font-semibold text-[#0f172a]">Total:</span><span className="text-xl font-bold text-[#10b981]">${getTotalPrice().toFixed(2)} USD</span></div></div>
                    </>
                  )}
                  {!discountApplied && (
                    <div className="flex justify-between items-center"><span className="text-lg font-semibold text-[#0f172a]">Total:</span><span className="text-xl font-bold text-[#10b981]">${getSubtotalPrice().toFixed(2)} USD</span></div>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="border-t p-4 space-y-3">
              <h3 className="font-medium text-[#0f172a] flex items-center gap-1"><span>👤</span> Datos del comprador</h3>
              <input type="text" placeholder="Nombre completo del comprador *" value={customerData.name} onChange={(e) => setCustomerData({...customerData, name: e.target.value})} className="w-full p-3 border rounded-full focus:outline-none focus:ring-2 focus:ring-[#10b981]" />
              <input type="tel" placeholder="Teléfono del comprador *" value={customerData.phone} onChange={(e) => setCustomerData({...customerData, phone: e.target.value})} className="w-full p-3 border rounded-full focus:outline-none focus:ring-2 focus:ring-[#10b981]" />
              
              <h3 className="font-medium text-[#0f172a] pt-4 border-t flex items-center gap-1"><span>🎁</span> Datos del beneficiario (quien recibe)</h3>
              <input type="text" placeholder="Nombre completo del beneficiario *" value={customerData.beneficiaryName} onChange={(e) => setCustomerData({...customerData, beneficiaryName: e.target.value})} className="w-full p-3 border rounded-full focus:outline-none focus:ring-2 focus:ring-[#10b981]" />
              <input type="tel" placeholder="Teléfono del beneficiario *" value={customerData.beneficiaryPhone} onChange={(e) => setCustomerData({...customerData, beneficiaryPhone: e.target.value})} className="w-full p-3 border rounded-full focus:outline-none focus:ring-2 focus:ring-[#10b981]" />
              
              <h3 className="font-medium text-[#0f172a] pt-4 border-t flex items-center gap-1"><span>📍</span> Entrega</h3>
              <textarea placeholder="Dirección de entrega *" value={customerData.address} onChange={(e) => setCustomerData({...customerData, address: e.target.value})} className="w-full p-3 border rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#10b981] h-20" />
              <textarea placeholder="Notas adicionales (opcional)" value={customerData.notes} onChange={(e) => setCustomerData({...customerData, notes: e.target.value})} className="w-full p-3 border rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#10b981] h-16" />
            </div>

            <div className="border-t p-4 space-y-3">
              <button onClick={handleProcessOrder} className="w-full bg-[#10b981] text-white py-3 rounded-full font-medium hover:bg-opacity-90 transition-all flex items-center justify-center gap-2"><span>✅</span> Procesar Pedido</button>
              <button onClick={onClose} className="w-full bg-gray-200 text-[#0f172a] py-3 rounded-full font-medium hover:bg-gray-300 transition-all">Continuar Comprando</button>
            </div>
          </>
        )}
      </div>
...
const SocialMediaLinks = React.memo(() => {
  return (
    <div className="flex items-center justify-center space-x-4 px-4 py-4 bg-white border-b border-gray-100">
      ...
      <a href="https://whatsapp.com/channel/0029Vb89B2bBfxo2tGL8Fk0G" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-[#25D366] flex items-center justify-center transform transition-all hover:scale-110 hover:shadow-lg" aria-label="WhatsApp">
        <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M12.04 2C6.58 2 2.13 6.45 2.13 11.91C2.13 13.79 2.7 15.6 3.73 17.13L2.05 21.83L6.88 20.18C8.38 21.08 10.09 21.59 11.99 21.59C17.45 21.59 21.9 17.14 21.9 11.68C21.9 6.22 17.5 2 12.04 2ZM12.04 4.13C16.5 4.13 19.77 7.46 19.77 11.73C19.77 16 16.45 19.33 12.04 19.33C10.36 19.33 8.78 18.8 7.43 17.91L7.18 17.76L4.44 18.66L5.36 16.02L5.18 15.75C4.22 14.32 3.66 12.65 3.66 11.73C3.66 7.46 7.04 4.13 12.04 4.13ZM8.67 8.18C8.48 8.18 8.19 8.25 7.94 8.53C7.69 8.81 7.04 9.51 7.04 10.93C7.04 12.35 8.09 13.71 8.24 13.92C8.39 14.13 10.1 16.88 12.7 16.03C14.16 15.54 14.62 14.56 14.74 14.12C14.86 13.68 14.86 13.3 14.81 13.2C14.76 13.1 14.61 13.05 14.41 12.95C14.21 12.85 13.31 12.4 13.13 12.34C12.95 12.28 12.81 12.25 12.67 12.45C12.53 12.65 12.13 13.15 12.01 13.29C11.89 13.43 11.77 13.45 11.57 13.35C11.37 13.25 10.78 13.04 10.09 12.42C9.55 11.95 9.18 11.36 9.06 11.16C8.94 10.96 9.05 10.84 9.15 10.74C9.24 10.65 9.35 10.5 9.45 10.38C9.55 10.26 9.59 10.18 9.67 10.04C9.75 9.9 9.71 9.78 9.65 9.68C9.59 9.58 9.21 8.68 9.05 8.28C8.9 7.9 8.74 7.95 8.62 7.94C8.51 7.93 8.37 7.93 8.23 7.93C8.09 7.93 7.86 8.18 8.67 8.18Z" /></svg>
      </a>
      <a href="https://www.instagram.com/tudespensa.25?igsh=MWt5dzZjcWh0NnF0MA==" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-gradient-to-br from-[#833ab4] via-[#fd1d1d] to-[#fcaf45] flex items-center justify-center transform transition-all hover:scale-110 hover:shadow-lg" aria-label="Instagram">
      ...
      <a href="https://www.facebook.com/share/16cchoNUTo/" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-[#1877f2] flex items-center justify-center transform transition-all hover:scale-110 hover:shadow-lg" aria-label="Facebook">
      ...
      <div className="px-4 py-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            <OptimizedImage src="https://app.trickle.so/storage/public/images/usr_0b2efdb2f0000001/2fc53855-ded6-40ff-959b-7a5c6077e0e3.svg" alt="TuDespensa25 Logo" className="w-12 h-12 bg-white rounded-full p-2 shadow-sm" />
            <h1 className="text-xl font-bold text-[#0f172a]">TuDespensa25</h1>
          </div>
          <div className="flex items-center space-x-2">
            <button onClick={onMunicipalityClick} className="flex items-center space-x-2 bg-[#10b981] bg-opacity-10 rounded-full px-4 py-2 hover:bg-opacity-20 transition-all">
              <span className="text-sm text-[#10b981]">📍</span>
              <span className="text-sm max-w-24 truncate text-[#0f172a]">{municipalities.find(m => m.id === selectedMunicipality)?.name.split(', ')[1] || 'Municipio'}</span>
              <span className="text-sm text-[#10b981]">▼</span>
            </button>
            <NotificationButton />
            <button onClick={onCartClick} className="relative w-10 h-10 bg-[#10b981] bg-opacity-10 rounded-full flex items-center justify-center hover:bg-opacity-20 transition-all">
              <span className="text-lg text-[#10b981]">🛒</span>
              {cartItems > 0 && <span className="absolute -top-1 -right-1 bg-[#f59e0b] text-white text-xs w-5 h-5 rounded-full flex items-center justify-center font-medium">{cartItems}</span>}
            </button>
          </div>
        </div>
        <div className="relative">
          <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-lg text-gray-400">🔍</span>
          <input type="text" placeholder="Buscar productos..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-3 rounded-full bg-gray-50 text-[#0f172a] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#10b981] focus:bg-white border border-gray-200" />
        </div>
      </div>
    </header>
  );
  ...
  return (
    <div className="px-4 py-6 bg-gradient-to-b from-white to-[#fafaf9]">
      <div className="max-w-3xl mx-auto text-center">
        <h2 className="text-2xl md:text-3xl font-bold text-[#0f172a] mb-3">Ayuda a tu familia desde la distancia, <br className="hidden sm:block" /><span className="text-[#10b981]">nosotros nos encargamos del resto</span></h2>
        <p className="text-base text-[#475569] mb-4 max-w-2xl mx-auto">Compra alimentos, esenciales y productos del hogar de forma rápida, segura y confiable. Tu familia lo recibe, tú te quedas tranquilo.</p>
        <div className="flex flex-wrap items-center justify-center gap-4 text-sm text-[#64748b]">
          <div className="flex items-center gap-1"><span className="text-[#10b981]">🚚</span> Entregas rápidas</div>
          <div className="flex items-center gap-1"><span className="text-[#10b981]">🛡️</span> Pagos seguros</div>
          <div className="flex items-center gap-1"><span className="text-[#10b981]">📋</span> Catálogo listo</div>
          <div className="flex items-center gap-1"><span className="text-[#10b981]">💬</span> Atención humana</div>
        </div>
      </div>
    </div>
  );
        ...
        {categories.map((category) => (
          ...
          >
            <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-1 transition-all ${
              ...
              <span className={`text-xl transition-transform ${selectedCategory === category.id ? 'scale-105' : ''}`}>{category.icon}</span>
            </div>
            <p className={`text-[10px] font-medium text-center transition-colors leading-tight ${
              selectedCategory === category.id ? 'text-[#10b981] font-semibold' : 'text-[#334155]'
            }`}>{category.name}</p>
          </button>
        ))}
      </div>
...
const FloatingWhatsAppButton = React.memo(() => {
  const handleWhatsAppClick = () => {
    ...
    window.open(`https://wa.me/5354066204?text=${encodeURIComponent(message)}`, '_blank');
  };
  return (
    <button onClick={handleWhatsAppClick} className="fixed bottom-6 right-6 w-14 h-14 bg-[#25D366] text-white rounded-full shadow-lg hover:shadow-xl transform hover:scale-110 transition-all duration-200 z-40 flex items-center justify-center" aria-label="WhatsApp">
      <svg className="w-7 h-7 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M12.04 2C6.58 2 2.13 6.45 2.13 11.91C2.13 13.79 2.7 15.6 3.73 17.13L2.05 21.83L6.88 20.18C8.38 21.08 10.09 21.59 11.99 21.59C17.45 21.59 21.9 17.14 21.9 11.68C21.9 6.22 17.5 2 12.04 2ZM12.04 4.13C16.5 4.13 19.77 7.46 19.77 11.73C19.77 16 16.45 19.33 12.04 19.33C10.36 19.33 8.78 18.8 7.43 17.91L7.18 17.76L4.44 18.66L5.36 16.02L5.18 15.75C4.22 14.32 3.66 12.65 3.66 11.73C3.66 7.46 7.04 4.13 12.04 4.13ZM8.67 8.18C8.48 8.18 8.19 8.25 7.94 8.53C7.69 8.81 7.04 9.51 7.04 10.93C7.04 12.35 8.09 13.71 8.24 13.92C8.39 14.13 10.1 16.88 12.7 16.03C14.16 15.54 14.62 14.56 14.74 14.12C14.86 13.68 14.86 13.3 14.81 13.2C14.76 13.1 14.61 13.05 14.41 12.95C14.21 12.85 13.31 12.4 13.13 12.34C12.95 12.28 12.81 12.25 12.67 12.45C12.53 12.65 12.13 13.15 12.01 13.29C11.89 13.43 11.77 13.45 11.57 13.35C11.37 13.25 10.78 13.04 10.09 12.42C9.55 11.95 9.18 11.36 9.06 11.16C8.94 10.96 9.05 10.84 9.15 10.74C9.24 10.65 9.35 10.5 9.45 10.38C9.55 10.26 9.59 10.18 9.67 10.04C9.75 9.9 9.71 9.78 9.65 9.68C9.59 9.58 9.21 8.68 9.05 8.28C8.9 7.9 8.74 7.95 8.62 7.94C8.51 7.93 8.37 7.93 8.23 7.93C8.09 7.93 7.86 8.18 8.67 8.18Z" /></svg>
    </button>
  );
});
...
const FloatingWishlistButton = React.memo(() => {
  ...
  return (
    <button onClick={handleWishlistClick} className="fixed bottom-24 right-6 w-14 h-14 bg-gradient-to-br from-[#833ab4] via-[#fd1d1d] to-[#fcaf45] text-white rounded-full shadow-lg hover:shadow-xl transform hover:scale-110 transition-all duration-200 z-40 flex items-center justify-center" aria-label="Lista de deseos">
...
const ProductCard = React.memo(({ product, onAddToCart, likedProducts, onToggleLike, onProductClick }) => {
  ...
  const handleCardClick = (e) => { if (e.target.closest('button')) return; onProductClick(product); };
  const handleLikeClick = (e) => { e.stopPropagation(); onToggleLike(product.id); };
  const handleAddToCart = (e) => { e.stopPropagation(); onAddToCart(product); };
  const handleShare = (e) => {
    e.stopPropagation();
    const shareUrl = generateShareableLink(product.id);
    navigator.clipboard.writeText(shareUrl).then(() => alert('¡Enlace copiado! Comparte este producto.')).catch(() => {
  ...
  return (
    <div className="product-card bg-white rounded-2xl border border-gray-100 overflow-hidden transition-all hover:shadow-lg hover:-translate-y-1 cursor-pointer" onClick={handleCardClick}>
      <div className="aspect-square bg-[#fafaf9] relative overflow-hidden p-4">
        <OptimizedImage src={product.image} alt={product.name} className="w-full h-full object-contain" />
        <div className="absolute top-2 right-2 flex flex-col space-y-1">
          <button onClick={handleLikeClick} className="w-8 h-8 bg-white bg-opacity-90 rounded-full flex items-center justify-center shadow-sm hover:scale-110 transition">
            <span className={`text-lg ${isLiked ? 'text-red-500' : 'text-gray-400'}`}>{isLiked ? '❤️' : '🤍'}</span>
          </button>
      ...
      <div className="p-4">
        <h3 className="font-semibold text-[#0f172a] text-sm mb-1 line-clamp-2">{product.name}</h3>
        <p className="text-xs text-[#64748b] mb-2 line-clamp-2">{product.description}</p>
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs px-3 py-1 bg-gray-100 text-[#334155] rounded-full font-medium flex items-center gap-1">{categories.find(c => c.id === product.category)?.icon} {categories.find(c => c.id === product.category)?.name}</span>
          <span className="text-lg font-bold text-[#10b981]">${originalPrice.toFixed(2)} <span className="text-xs font-normal text-[#64748b]">USD</span></span>
        </div>
        <button onClick={handleAddToCart} className="w-full bg-[#10b981] text-white py-3 px-4 rounded-full text-sm font-semibold flex items-center justify-center space-x-2 hover:bg-[#059669] transition-all shadow-sm hover:shadow">
          <span>🛒</span><span>Añadir al carrito</span>
        </button>
      </div>
    </div>
  );
...
const ProductDetailModal = React.memo(({ isOpen, onClose, product, onAddToCart, likedProducts, onToggleLike }) => {
  ...
  const handleShare = () => {
    if (product) {
      const shareUrl = generateShareableLink(product.id);
      if (navigator.share) {
        navigator.share({ title: product.name, text: product.description, url: shareUrl }).catch(console.error);
      } else {
        navigator.clipboard.writeText(shareUrl).then(() => alert('¡Enlace copiado al portapapeles! Compártelo con quien quieras.')).catch(() => {
          const tempInput = document.createElement('input');
          tempInput.value = shareUrl;
          document.body.appendChild(tempInput);
          tempInput.select();
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-end sm:items-center sm:justify-center">
      <div className="bg-white w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-t-3xl sm:rounded-3xl">
        <div className="relative bg-[#fafaf9] p-6">
          <OptimizedImage src={product.image} alt={product.name} className="w-full h-auto max-h-[400px] object-contain" />
          <button onClick={onClose} className="absolute top-4 right-4 w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-md hover:scale-110 transition"><span className="text-lg text-gray-600">✕</span></button>
          <button onClick={() => onToggleLike(product.id)} className="absolute top-4 left-4 w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-md hover:scale-110 transition"><span className={`text-lg ${isLiked ? 'text-red-500' : 'text-gray-400'}`}>{isLiked ? '❤️' : '🤍'}</span></button>
        </div>
        <div className="p-6">
          <div className="flex items-start justify-between mb-4">
            <div className="flex-1">
              <h2 className="text-xl font-bold text-[#0f172a] mb-2">{product.name}</h2>
              <span className="inline-block px-3 py-1 bg-gray-100 text-[#334155] rounded-full text-xs font-medium flex items-center gap-1 w-fit">{categoryIcon} {categoryName}</span>
            </div>
            <div className="text-right ml-4">
              <span className="text-2xl font-bold text-[#10b981]">${product.price.toFixed(2)}</span>
              <span className="block text-xs text-[#64748b]">USD</span>
            </div>
          </div>
          <div className="mb-6">
            <h3 className="text-lg font-semibold text-[#0f172a] mb-2 flex items-center gap-1"><span>📝</span> Descripción</h3>
            <p className="text-[#475569] leading-relaxed">{product.description}</p>
          </div>
          <div className="space-y-3">
            <button onClick={() => { onAddToCart(product); onClose(); }} className="w-full bg-[#10b981] text-white py-3 rounded-full font-semibold hover:bg-[#059669] transition-all shadow-sm hover:shadow flex items-center justify-center gap-2"><span>🛒</span> Añadir al carrito</button>
            <button onClick={handleShare} className="w-full bg-white text-[#10b981] border-2 border-[#10b981] py-3 rounded-full font-semibold flex items-center justify-center space-x-2 hover:bg-[#f0fdf4] transition-all"><span>🔗</span><span>Compartir Producto</span></button>
            <button onClick={onClose} className="w-full bg-gray-100 text-[#0f172a] py-3 rounded-full font-medium hover:bg-gray-200 transition-all">Cerrar</button>
          </div>
        </div>
      </div>
    ...
    <footer className="bg-[#0f172a] text-white mt-12 rounded-t-3xl px-6 py-10">
      <div className="text-center mb-8">
        <div className="flex items-center justify-center space-x-2 mb-4">
          <div className="w-12 h-12 bg-gradient-to-r from-[#10b981] to-[#059669] rounded-full flex items-center justify-center shadow-lg"><span className="text-xl text-white">🛒</span></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center text-sm mb-8">
        <div className="bg-white/5 rounded-2xl p-4"><span className="text-3xl text-[#10b981] mb-2 block">🚚</span><h4 className="font-semibold text-[#10b981] mb-1">Entrega rápida</h4><p className="text-gray-300 text-xs">24-48 horas</p></div>
        <div className="bg-white/5 rounded-2xl p-4"><span className="text-3xl text-[#10b981] mb-2 block">🛡️</span><h4 className="font-semibold text-[#10b981] mb-1">Pago seguro</h4><p className="text-gray-300 text-xs">Transferencia Zelle</p></div>
        <div className="bg-white/5 rounded-2xl p-4"><span className="text-3xl text-[#10b981] mb-2 block">⭐</span><h4 className="font-semibold text-[#10b981] mb-1">Soporte</h4><p className="text-gray-300 text-xs">WhatsApp directo</p></div>
      </div>
      <div className="border-t border-gray-800 pt-6 text-center"><p className="text-xs text-gray-400">© 2025 TuDespensa.25. Todos los derechos reservados.</p></div>
    </footer>
  );
});
  ...
  return (
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
      {products.map((product) => (
        <ProductCard key={product.id} product={product} onAddToCart={onAddToCart} likedProducts={likedProducts} onToggleLike={onToggleLike} onProductClick={onProductClick} />
      ))}
    </div>
  );
});
...
const FAQSection = React.memo(() => {
  ...
  const toggleItem = (index) => setOpenItems(prev => prev.includes(index) ? prev.filter(i => i !== index) : [...prev, index]);
  const faqItems = [
    { question: "¿Qué métodos de pago aceptan?", answer: "💳 **Solo Zelle** para clientes desde el exterior. El pago se verifica al instante, sin impuestos adicionales. Pagas seguro y al momento, desde donde estés." },
    { question: "¿Cuál es el tiempo de entrega?", answer: "🚚 **Entregas en menos de 48 horas**, y la mayoría de los pedidos llegan el mismo día. Esto aplica en toda la provincia Artemisa." },
    { question: "¿Cuánto cuesta el envío?", answer: `📦 **Tarifa de Envíos:**\n\n✅ **Municipios Gratis** (a partir de 30 USD, menos → 1.99 USD):\n• San Cristóbal\n• Candelaria\n• Guanajay\n\n✅ **Envío 3.99 USD fijo:**\n• Artemisa (cabecera)\n• Caimito\n• Mariel\n\n✅ **Envío 5 USD fijo:**\n• Bauta\n• Güira de Melena\n• San Antonio de los Baños\n• Bahía Honda\n• Alquízar` },
    { question: "¿En qué municipios realizan entregas?", answer: "📍 Cubrimos **toda la provincia Artemisa**: San Cristóbal, Candelaria, Mariel, Guanajay, Caimito, Bauta, Artemisa, Alquízar, Güira de Melena, San Antonio de los Baños y Bahía Honda." },
    { question: "¿Qué categorías de productos ofrecen?", answer: `🛍️ **Nuestras Categorías:**\n\n🥫 **Alimentos:** arroz, granos, carnes, lácteos, dulces, conservas\n🧼 **Productos de Aseo y Limpieza:** detergentes, jabones, higiene personal\n⚡ **Electrodomésticos:** lavadoras, batidoras, fogones, freezers\n🎁 **Combos especiales** con descuentos exclusivos` },
    { question: "¿Tienen promociones especiales?", answer: "🎯 **Promociones Activas:**\n\n🔥 **Combos Especiales:** 3 días a la semana mostramos combos con descuentos\n💥 **Zona Roja:** Productos con descuentos mayores para aprovechar\n📦 **Envío Gratis:** En municipios seleccionados a partir de 30 USD" },
    { question: "¿Puedo hacer pedidos personalizados?", answer: "✅ **¡Sí!** Ofrecemos la posibilidad de crear pedidos personalizados y combos especiales según tus necesidades. Contáctanos por WhatsApp para coordinar tu pedido a medida." },
    { question: "¿Qué hago si un producto no está disponible?", answer: "🔄 Si un producto no está disponible, te contactaremos inmediatamente para ofrecerte alternativas similares de igual o mejor calidad, o programar la entrega una vez que tengamos stock." },
    { question: "¿Proveen comprobante de compra?", answer: "🧾 **Sí, absolutamente.** Proveemos comprobante de venta por todos nuestros pedidos. Solo debes solicitarlo al momento de hacer tu pedido y te lo entregaremos con tu compra." },
    { question: "¿Qué es TuDespensa.25?", answer: "🏪 **TuDespensa.25** es tu tienda online de confianza en Artemisa, especializada en la venta y distribución de productos de primera necesidad con entrega a domicilio rápida y precios competitivos." }
  ];
  return (
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-10">
          <div className="flex items-center justify-center space-x-3 mb-4">
            <div className="w-16 h-16 bg-gradient-to-r from-[#10b981] to-[#059669] rounded-full flex items-center justify-center shadow-lg"><span className="text-2xl text-white">❓</span></div>
            <div><h2 className="text-3xl font-bold text-[#0f172a]">Preguntas Frecuentes</h2><p className="text-[#64748b] mt-1">Todo lo que necesitas saber sobre TuDespensa.25</p></div>
          </div>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          {faqItems.map((item, index) => (
            <div key={index} className="border border-gray-200 rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-md bg-white">
              <button onClick={() => toggleItem(index)} className="w-full px-6 py-4 text-left flex items-center justify-between bg-white hover:bg-gray-50 transition-colors">
                <span className="font-semibold text-[#0f172a] text-sm sm:text-base pr-4">{item.question}</span>
                <span className={`transform transition-transform duration-300 flex-shrink-0 text-[#10b981] text-lg ${openItems.includes(index) ? 'rotate-180' : ''}`}>▼</span>
              </button>
              {openItems.includes(index) && (
                <div className="px-6 py-4 bg-gray-50 border-t border-gray-100">
                  <div className="text-[#475569] text-sm sm:text-base leading-relaxed whitespace-pre-line">{item.answer}</div>
                </div>
              )}
            </div>
          ))}
        ...
        <div className="mt-12 text-center bg-gradient-to-r from-[#10b981] to-[#059669] rounded-3xl p-8 text-white shadow-xl">
          ...
          <p className="text-white text-opacity-90 mb-6 text-lg">Estamos aquí para ayudarte personalmente</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="https://wa.me/5354066204" target="_blank" rel="noopener noreferrer" className="bg-white text-[#10b981] px-8 py-4 rounded-full font-bold flex items-center justify-center space-x-3 hover:scale-105 transition-transform shadow-lg"><span className="text-xl">💬</span><span className="text-lg">WhatsApp Directo</span></a>
            <a href="tel:+5354066204" className="bg-transparent border-2 border-white text-white px-8 py-4 rounded-full font-bold flex items-center justify-center space-x-3 hover:bg-white hover:text-[#10b981] transition-all"><span className="text-xl">📞</span><span className="text-lg">Llamar Ahora</span></a>
          </div>
          <p className="text-white text-opacity-80 mt-4 text-sm">Horario de atención: 24/7 • Respuesta inmediata</p>
        </div>
      </div>
    </section>
  );
    ...
    if (this.state.hasError) {
      return (
        <div className="min-h-screen flex items-center justify-center bg-gray-50">
          <div className="text-center"><h1 className="text-2xl font-bold text-gray-900 mb-4">Algo salió mal</h1><p className="text-gray-600 mb-4">Lo sentimos, ocurrió un error inesperado.</p><button onClick={() => window.location.reload()} className="px-6 py-2 bg-[#10b981] text-white rounded-full">Recargar Página</button></div>
        </div>
      );
    }
...
function App() {
  ...
  const filteredProducts = React.useMemo(() => {
    ...
    filtered = filtered.filter(product => isProductAvailable(product, selectedMunicipality));
    if (selectedCategory !== 'todos') filtered = filtered.filter(product => product.category === selectedCategory);
    if (debouncedSearchTerm.trim()) {
      const term = debouncedSearchTerm.toLowerCase().trim();
      filtered = filtered.filter(product => product.name.toLowerCase().includes(term) || product.description.toLowerCase().includes(term));
    }
    return filtered;
  ...
  const handleAddToCart = (product) => {
    ...
    setNotification({ message: `${product.name} ha sido añadido al carrito`, isVisible: true });
  };

  const handleToggleLike = (productId) => setLikedProducts(prev => prev.includes(productId) ? prev.filter(id => id !== productId) : [...prev, productId]);
  ...
  const handleProcessOrder = (customerData, discountCode = '', discountPercentage = 0, discountAmount = 0) => {
    ...
    let orderMessage = `*NUEVO PEDIDO - TuDespensa.25*\n\n*DATOS DEL COMPRADOR:*\n👤 Nombre: ${customerData.name}\n📱 Teléfono: ${customerData.phone}\n\n*DATOS DEL BENEFICIARIO:*\n👤 Nombre: ${customerData.beneficiaryName}\n📱 Teléfono: ${customerData.beneficiaryPhone}\n\n*DATOS DE ENTREGA:*\n📍 Dirección: ${customerData.address}\n🏘️ Municipio: ${selectedMunicipalityName}\n${customerData.notes ? `📝 Notas: ${customerData.notes}\n` : ''}`;
    if (discountCode && discountPercentage > 0) orderMessage += `\n*🎫 DESCUENTO APLICADO:*\nCódigo: ${discountCode}\nDescuento: ${discountPercentage}%\n💵 Ahorro: $${discountAmount.toFixed(2)} USD\n\n`;
    orderMessage += `\n*PRODUCTOS SOLICITADOS:*\n`;
    let subtotal = 0;
    cart.forEach((item, index) => {
      const itemTotal = item.price * item.quantity;
      subtotal += itemTotal;
      orderMessage += `${index + 1}. ${item.name}\n   💰 Precio: $${item.price.toFixed(2)} USD\n   📦 Cantidad: ${item.quantity}\n   💵 Subtotal: $${itemTotal.toFixed(2)} USD\n\n`;
    });
    orderMessage += `*RESUMEN DE PAGO:*\n`;
    if (discountPercentage > 0) {
      orderMessage += `🛒 Subtotal: $${subtotal.toFixed(2)} USD\n🎫 Descuento (${discountPercentage}%): -$${discountAmount.toFixed(2)} USD\n💰 *TOTAL FINAL: $${(subtotal - discountAmount).toFixed(2)} USD*\n\n`;
    } else {
    ...
    window.open(`https://wa.me/5354066204?text=${encodeURIComponent(orderMessage)}`, '_blank');
    setCart([]);
    setIsCartOpen(false);
  };
  ...
  return (
    <div className="min-h-screen bg-[#fafaf9]">
      <Header searchTerm={searchTerm} setSearchTerm={setSearchTerm} selectedMunicipality={selectedMunicipality} setSelectedMunicipality={setSelectedMunicipality} cartItems={getTotalCartItems()} onCartClick={() => setIsCartOpen(true)} onMunicipalityClick={() => setShowMunicipalityModal(true)} />
      <SocialMediaLinks />
      <HeroText />
      <main className="pb-20">
        ...
        <div className="px-4 mt-2 products-section">
          <h2 className="text-xl font-bold text-[#0f172a] mb-4">{selectedCategory === 'todos' ? 'Todos los Productos' : categories.find(c => c.id === selectedCategory)?.name}</h2>
          <ProductGrid products={filteredProducts} onAddToCart={handleAddToCart} likedProducts={likedProducts} onToggleLike={handleToggleLike} onProductClick={handleProductClick} />
        </div>
        <FAQSection />
      </main>
      <Footer />
      ...
      <NotificationToast message={notification.message} isVisible={notification.isVisible} onClose={() => setNotification({ message: '', isVisible: false })} />
      <CartModal isOpen={isCartOpen} onClose={() => setIsCartOpen(false)} cart={cart} setCart={setCart} onProcessOrder={(customerData, discountCode, discountPercentage, discountAmount) => handleProcessOrder(customerData, discountCode, discountPercentage, discountAmount)} />
      <ProductDetailModal isOpen={!!selectedProduct} onClose={handleCloseProductModal} product={selectedProduct} onAddToCart={handleAddToCart} likedProducts={likedProducts} onToggleLike={handleToggleLike} />
      <MunicipalityModal isOpen={showMunicipalityModal} onClose={() => setShowMunicipalityModal(false)} selectedMunicipality={selectedMunicipality} setSelectedMunicipality={setSelectedMunicipality} />
    </div>
  );
}
